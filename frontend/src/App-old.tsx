...
import { Upload, FileText, MessageSquare, Zap, BarChart3, CheckCircle, Save, RotateCcw } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    const draft = localStorage.getItem('nyct-decline-draft');
    setHasDraft(!!draft);
  }, []);
...
NYCT Program Team`,
  ...
  const saveDraft = () => {
    ...
    localStorage.setItem('nyct-decline-draft', JSON.stringify(draft));
    setDraftSaved(true);
  ...
  const loadDraft = () => {
    ...
    if (draft) {
      ...
      if (parsed.selectedReasonCode) setSelectedReasonCode(parsed.selectedReasonCode);
  ...
  const resetForm = () => {
    ...
    localStorage.removeItem('nyct-decline-draft');
  };
          ...
          <div className="space-y-6">
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                <span className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold bg-gradient-to-r from-primary-100 to-accent-100 text-primary-800">
                  Generate Professional Decline Rationale
                </span>
                <div className="text-sm text-slate-600 font-medium">
                  {uploadedFile && selectedReasonCode && staffNote.trim() ? 'Ready to Generate' : `Step ${uploadedFile ? (selectedReasonCode ? (staffNote.trim() ? '4' : '3') : '2') : '1'} of 4`}
                </div>
              </div>
              ...
              <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden">
                <div 
                  className="h-full bg-gradient-to-r from-primary-500 to-accent-500 rounded-full transition-all duration-500 ease-in-out"
                  style={{ 
                    width: `${uploadedFile ? (selectedReasonCode ? (staffNote.trim() ? '100' : '75') : '50') : '25'}%`
                  }}
                ></div>
              </div>
            </div>
            ...
            {/* Draft Management */}
            {(hasDraft || uploadedFile || selectedReasonCode || staffNote.trim()) && !generatedOutput && (
              <div className="bg-gradient-to-r from-accent-50/50 to-primary-50/50 border border-accent-200 rounded-xl p-4 mb-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="bg-accent-100 p-2 rounded-lg">
                      <Save className="h-4 w-4 text-accent-600" />
                    </div>
                    <div>
                      <h3 className="text-sm font-semibold text-slate-900">
                        {hasDraft ? 'Resume Previous Work' : 'Save Your Progress'}
                      </h3>
                      <p className="text-xs text-slate-600">
                        {hasDraft ? 'Continue working on your saved draft' : 'Keep your work safe while you step away'}
                      </p>
                    </div>
                  </div>
                  <div className="flex space-x-2">
                    {hasDraft && (
                      <button
                        onClick={loadDraft}
                        className="inline-flex items-center space-x-2 px-3 py-2 text-xs font-medium text-accent-700 bg-accent-100 hover:bg-accent-200 rounded-lg transition-all duration-200"
                      >
                        <RotateCcw className="h-3 w-3" />
                        <span>Load Draft</span>
                      </button>
                    )}
                    {(uploadedFile || selectedReasonCode || staffNote.trim()) && (
                      <button
                        onClick={saveDraft}
                        className={`inline-flex items-center space-x-2 px-3 py-2 text-xs font-medium rounded-lg transition-all duration-200 ${
                          draftSaved 
            ...
            {/* Step 1: Upload */}
            <div className={`bg-white/80 backdrop-blur-sm rounded-xl shadow-card border p-6 hover:shadow-elevated transition-all duration-300 ${
              uploadedFile ? 'border-success-300 bg-success-50/30' : 'border-slate-200/60'
            }`}>
              <div className="flex items-center mb-4">
                <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-semibold mr-3 transition-all duration-200 ${
                  uploadedFile 
                    ? 'bg-success-100 text-success-700' 
                    : 'bg-primary-100 text-primary-600'
                }`}>
                  {uploadedFile ? <CheckCircle className="h-5 w-5" /> : '1'}
                </div>
            ...
            {/* Step 2: Reason */}
            <div className={`bg-white/80 backdrop-blur-sm rounded-xl shadow-card border p-6 hover:shadow-elevated transition-all duration-300 ${
              selectedReasonCode ? 'border-success-300 bg-success-50/30' : uploadedFile ? 'border-primary-300 bg-primary-50/30' : 'border-slate-200/60 opacity-60'
            }`}>
              <div className="flex items-center mb-4">
                <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-semibold mr-3 transition-all duration-200 ${
                  selectedReasonCode 
                  ...
                  {selectedReasonCode ? <CheckCircle className="h-5 w-5" /> : '2'}
                </div>
                <h2 className="text-lg font-semibold text-slate-900 flex items-center">
            ...
            {/* Step 3: Staff Note */}
            <div className={`bg-white/80 backdrop-blur-sm rounded-xl shadow-card border p-6 hover:shadow-elevated transition-all duration-300 ${
              staffNote.trim() ? 'border-success-300 bg-success-50/30' : selectedReasonCode ? 'border-primary-300 bg-primary-50/30' : 'border-slate-200/60 opacity-60'
            }`}>
              <div className="flex items-center mb-4">
                <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-semibold mr-3 transition-all duration-200 ${
                  staffNote.trim() 
                  ...
                  {staffNote.trim() ? <CheckCircle className="h-5 w-5" /> : '3'}
                </div>
